...
bool visitedDFS[CA + 1][CB + 1][CC + 1];
...
bool dfs(State cur, State start) {
    if (isGoal(cur)) {
        printf("Goal reached (DFS): (%d, %d, %d)\n",
               cur.a, cur.b, cur.c);
        printPath(cur, start);
        return true;
    }

    visitedDFS[cur.a][cur.b][cur.c] = true;
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            if (i != j) {
                ...
                if (!visitedDFS[next.a][next.b][next.c]) {
                    parent[next.a][next.b][next.c] = cur;
                    parentAction[next.a][next.b][next.c] = (Action){i, j};
